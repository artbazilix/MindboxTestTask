...
namespace GeometryTests
{
    ...
    public class TriangleTests
    {
        ...
        public void GetArea_ReturnsCorrectAreaForRandomSides()
        {
            ...
            for (int i = 0; i < 100; i++)
            {
                var sideA = random.NextDouble() * 100;
                var sideB = random.NextDouble() * 100;
                var sideC = random.NextDouble() * 100;
                var triangle = new Triangle { SideA = sideA, SideB = sideB, SideC = sideC };
                var s = (sideA + sideB + sideC) / 2;
                var expectedArea = Math.Sqrt(s * (s - sideA) * (s - sideB) * (s - sideC));
                Assert.AreEqual(expectedArea, triangle.GetArea(), 1e-10);
            }
        }
        ...
        {
            var triangle = new Triangle { SideA = 3.0, SideB = 4.0, SideC = 5.0 };
