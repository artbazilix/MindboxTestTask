...
namespace GeometryTests
{
    ...
    public class RectangleTests
    {
        ...
        public void GetArea_ReturnsCorrectArea()
        {
            ...
            var expectedArea = 12; // Площадь прямоугольника равна a * b
        ...
        public void GetArea_ReturnsCorrectAreaForRandomLengthAndWidth()
        {
            ...
            for (int i = 0; i < 100; i++)
            {
                var length = random.NextDouble() * 100; // Случайная длина от 0 до 100
                var width = random.NextDouble() * 100; // Случайная ширина от 0 до 100
                var rectangle = new Rectangle { Length = length, Width = width };
                var expectedArea = length * width;
                Assert.AreEqual(expectedArea, rectangle.GetArea(), 1e-10); // Допускаем небольшую погрешность
            }
        }
